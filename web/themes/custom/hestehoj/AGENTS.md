# Hestehoj Theme - Agent Rules

This document contains coding rules and conventions for the Hestehoj theme that AI agents should follow when making changes.
...
**Note**: Even though Drupal core templates sometimes use `<div{{ attributes }}>`, this pattern is not allowed in Hestehoj theme components.
...
{{
  include(
    '@hestehoj/components/icon/icon.twig',
    {
      weight: 'bold',
      icon: icon
    }
...
```twig
{% include '@hestehoj/components/icon/icon.twig' with {
  weight: 'bold',
  icon: icon
} %}
```
...
**Rule**: After completing any changes to the Hestehoj theme, always run `npm run format` and `npm run build` to ensure code is properly formatted and the build artifacts are up to date.
...
**Note**: These commands should be run from the Hestehoj theme directory (`web/themes/custom/hestehoj/`). Running these commands ensures that:
