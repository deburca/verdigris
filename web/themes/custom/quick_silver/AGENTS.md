# QuickSilver Theme - Agent Rules

This document contains coding rules and conventions for the QuickSilver theme that AI agents should follow when making changes.
...
**Note**: Even though Drupal core templates sometimes use `<div{{ attributes }}>`, this pattern is not allowed in QuickSilver theme components.
...
{{
  include(
    '@quick_silver/components/icon/icon.twig',
    {
      weight: 'bold',
      icon: icon
    }
...
```twig
{% include '@quick_silver/components/icon/icon.twig' with {
  weight: 'bold',
  icon: icon
} %}
```
...
**Rule**: After completing any changes to the QuickSilver theme, always run `npm run format` and `npm run build` to ensure code is properly formatted and the build artifacts are up to date.
...
**Note**: These commands should be run from the QuickSilver theme directory (`web/themes/custom/quick_silver/`). Running these commands ensures that:
